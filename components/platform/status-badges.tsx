...
const taskStatusMap: Record<TaskStatus, { label: string; className: string }> = {
  ...
  queued: { label: "排队中", className: "text-blue-600" },
...
export function AccountStatusBadge({ status }: { status: AccountStatus }) {
  ...
  const colors = { active: "bg-emerald-100 text-emerald-700", locked: "bg-red-100 text-red-700", disabled: "bg-slate-100 text-slate-600" }
...
export function LogResultBadge({ result }: { result: LogResult }) {
  ...
  const colors = { success: "bg-emerald-100 text-emerald-700", failure: "bg-red-100 text-red-700", warning: "bg-amber-100 text-amber-700" }
