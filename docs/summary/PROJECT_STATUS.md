...
**关键交付**:
- `pnpm env:init` / `pnpm env:check` / `pnpm env:check:production` (环境配置硬约束)
- `lib/db/postgres.ts` 移除硬编码 fallback，缺 DATABASE_URL 直接 fail-closed
- 全站 `/volumes` 用户入口 → `/racks?view=volumes` (dashboard stats cards + command center)
- 用户可见 "Agent" → "站点代理" (8 文件, 12 处替换)
- 移除 `mock` as displayed data source type
- `components/check/__tests__/self-check.ts` 删除 (旧 17-tab /check 页已 redirect)
- `scripts/e2e/test-volumes.ts` 重写, 测试 /racks?view=volumes
- 6 个 e2e 测试更新对 merged view 的引用
- `test-login.js` 临时文件删除
- `docs/summary/DEPLOYMENT_GUIDE.md` 归档到 `docs/archive/`
- 7 个过期 sprint 脚本中立化
- `docs/operations/deployment.md` 新增 env init/check + Release Gate 章节
- `README.md` 快速启动增加 env:init, 本地验证增加 env:check

**真实状态**:
- §2.3 业务同步 → `complete`
- §4.2 任务控制 → `partial` + `blocked_by_site_change` (R.88 契约已落地, 站点代理未接入)
- §5.2 文件索引 → `partial` (R.85 port + R.86 调度, R.87 cron/监控未做)
- §2.2/§3.x 登录 / RBAC / SSO → `blocked_by_auth`

**未实现 (留待后续 Sprint / 领导决策)**:
- 站点 app poll control_command 队列 (需站点 app 团队配合)
- 站点库 schema 改造 (tbl_task.paused / priority 字段)
- 真实站点控制执行 (task → station DB 写入)
- 企业 SSO / LDAP 真实接入 (需 IdP 配置)
