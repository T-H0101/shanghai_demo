...
| REQ-2.3.2 | 同步策略 (实时/定时/手动) | `partial` | R.19C Agent 支持 5 秒任务轮询、60 秒小表快照、`--once` 手动同步、可配置自动重试、离线 spool 和恢复补传；R.21 失败 package/table 已聚合为真实同步告警 | 设备变更实时性实测；最终失败后的通知通道和责任人推送；页面触发 Agent 手动全量/增量；生产部署 | R.22 页面手动触发 Agent 全量/增量 + 同步延迟计时 |
