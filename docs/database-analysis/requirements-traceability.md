...
- REQ-1.2.1: heartbeat 已完成请求级 HMAC 和持久化；独立 Agent 与控制生产链路尚未部署。
...
| REQ-1.2.1 | 松耦合 (API/MQ 交互, 不侵入站点核心逻辑) | `partial` | package API 松耦合；R.19A heartbeat 已有请求级 HMAC、nonce 防重放和持久化；独立 Agent 与控制 poll/ack/result 未部署 | 独立 Site Agent；生产控制路径移除中心库直连；HTTP control poll/ack/result | R.19B 实现独立 Agent |
...
| REQ-2.1.3 | 站点监控 (实时 + 告警, 采集 ≤5 分钟) | `partial` | 签名 heartbeat 可持久化并按 5/15 分钟计算 Agent 状态；无长期 Agent、硬件指标和自动告警 | 周期 heartbeat；硬件指标；自动告警 | R.19B 独立 Agent + R.20 告警源表 |
...
| REQ-2.3.2 | 同步策略 (实时/定时/手动) | `partial` | 已有手动 trigger、定时 scheduler、HMAC package push 和 Agent heartbeat；缺独立 Agent 实时推送、自动重试告警和离线补传 | 关键数据实时同步；可配置自动重试；最终失败告警；站点离线队列和补传 | R.19 Agent + R.21 同步策略闭环 |
...
| REQ-6.4.2 | 监控 (CPU/内存/磁盘/接口) | `partial` | 中心健康 API 和站点 Agent databaseReachable heartbeat 真实可读 | CPU/内存/磁盘完整监控；历史趋势与告警 | 接入真实主机指标源后补 CPU/内存/磁盘与趋势 |
