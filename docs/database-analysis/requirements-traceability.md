...
- REQ-6.1.3: R.23 `e2e:site-agent-sync` 增加任务增量 Agent→中心落库耗时断言，本次白盒样本 <100ms ≤10s；不替代生产持续样本和百万级全量验收。
- REQ-6.1.1: 没有满足 requirements 规模的性能报告。
- REQ-6.2.1: HMAC 不等于敏感字段加密。
- REQ-6.3.1: 尚未完成 Firefox/Edge 实测。
- REQ-2.2.1 / REQ-2.2.3 / REQ-6.2.2 / REQ-6.2.4: R.26 已解锁平台 local Auth 基座，完成服务端 JWT cookie、密码哈希、登录审计、失败锁定和 RBAC 权限目录；企业 ADFS/LDAP 直连、站点 SSO、权限分配 UI 和业务 API 全量权限过滤仍未完成。
...
| REQ-2.2.1 | ADFS 3.0+ / LDAP 集成登录 | `partial` | R.26 已完成中心平台 local 登录、HttpOnly JWT cookie、`/api/auth/me`、`/api/auth/logout`、RBAC 权限目录和服务端密码哈希；未直连企业 ADFS/LDAP | 企业 ADFS/LDAP 参数、集团账号属性映射、站点 SSO federation | 获得企业 IdP 参数后实现 oidc/ldap provider adapter |
| REQ-2.2.2 | 集团 AD ↔ 站点本地账号映射 | `blocked_by_auth` | 0 | CLAUDE.md 禁 + 源端无 AD 通道 | 解锁 CLAUDE.md (Sprint 5.x) — R.4 不再标 out_of_scope (违反 R.1 §1) |
| REQ-2.2.3 | 登录审计 (≥1 年) + 失败 ≥5 次锁定 | `partial` | R.26 服务端写 `auth_login_audit`，记录 success/failed/locked/logout；连续失败 >=5 次锁定 | 管理员解锁 UI、登录审计检索/导出 UI、锁定阈值页面配置 | 补登录审计管理页/API 后再评 complete |
...
| REQ-4.2.4 | 任务监控: 进度/状态/告警 push (≤10s 刷新) | `partial` | R.19E 将真实 control_command 状态并入任务中心，支持 URL 双视图和 5 秒命令刷新；R.25 悬浮助手新增全局 pending command / active alert / site agent 概览，去掉 mock 通知。 | 任务状态 <=10 秒同步证明；失败/超时告警；邮箱通知 | R.21 实时同步，R.25 全局监控入口 |
...
| REQ-5.1.2 | 日志导出 (Excel/CSV + 数字签名) | `partial` | R.24 `/api/logs/export` 已真实支持 CSV/JSON/XLSX；XLSX 为真实 OOXML 文件，manifest 含 signature 元数据边界，未配置密钥时显式 `blocked_by_config`；`/logs` 页面已接真实导出 | 证书或私钥数字签名；大数据分片/异步导出；两年留存策略；刻录/回迁全量业务日志字段 | 设计签名密钥托管、异步分片与留存策略后继续 |
...
| REQ-6.1.3 | 数据同步时效 (增量 ≤10s / 全量 ≤30min) | `partial` | R.19C Agent 默认每 5 秒读取任务增量，每 60 秒检测小表快照；R.23 白盒增量同步样本 <100ms ≤10s | 生产持续时延样本；百万级索引全量测试；生产站点长期运行证据 | R.25 生产持续时延和百万级全量性能验收 |
...
| REQ-6.2.2 | 存储加密 (不可逆 + 分区隔离) | `partial` | R.26 平台 `auth_accounts` 仅保存 scrypt 不可逆密码哈希；未实现 PG TDE、分区隔离或字段级加密 | PG TDE/字段级加密、分区隔离策略 | 设计存储加密和分区隔离策略 |
| REQ-6.2.3 | 操作审计 (不可篡改) | `partial` | 同步层日志、控制/导出 `audit_log` 与 R.26 登录审计均已落库 | 全部业务写操作统一审计、不可篡改存证、登录审计 UI | 补业务写操作审计覆盖率和防篡改方案 |
| REQ-6.2.4 | 防越权 (跨站/跨部门) | `partial` | R.26 建立平台角色、权限目录和 session permissions；尚未把跨站/跨部门过滤应用到所有业务查询 | 所有业务 API 权限中间件、跨站/跨部门数据过滤、权限分配 UI 与审计 | 为关键 API 接入 requirePermission/requireSiteAccess |
...
| REQ-6.4.2 | 监控 (CPU/内存/磁盘/接口) | `partial` | 中心健康 API、站点 Agent databaseReachable heartbeat 和 R.21 同步失败告警摘要真实可读；R.25 悬浮助手全局接入 `/api/system/health`、`/api/system/db-health`、`/api/alerts`、`/api/control/commands`、`/api/sync/sites/status`，不再显示假 CPU/内存百分比 | CPU/内存/磁盘完整监控；历史趋势与主机指标告警 | 接入真实主机指标源后补 CPU/内存/磁盘与趋势 |
