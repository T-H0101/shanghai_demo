...
- REQ-1.2.1: 独立 heartbeat Agent 已完成；package client、控制生产链路和生产部署尚未完成。
...
| REQ-1.2.1 | 松耦合 (API/MQ 交互, 不侵入站点核心逻辑) | `partial` | package API 松耦合；R.19A 总控 heartbeat 完成；R.19B 独立 Agent 已探活站点 DB 并主动回传 | 生产部署；package client；生产控制路径移除中心库直连；HTTP control poll/ack/result | R.19C package + R.19D control |
...
| REQ-2.1.3 | 站点监控 (实时 + 告警, 采集 ≤5 分钟) | `partial` | 独立 Agent 默认 5 分钟 heartbeat 并探活站点 DB；尚未生产部署，无硬件指标和自动告警 | 生产长期部署；硬件指标；自动告警 | 站点部署 + R.20 告警源表 |
...
| REQ-2.3.2 | 同步策略 (实时/定时/手动) | `partial` | 已有手动 trigger、定时 scheduler、package API 和独立 heartbeat Agent；缺 Agent package push、自动重试告警和离线补传 | 关键数据实时同步；可配置自动重试；最终失败告警；站点离线队列和补传 | R.19C + R.21 |
