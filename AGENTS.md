...
> 本文件完整镜像根目录 `CLAUDE.md`，并在文末追加 Codex 专属补充规则。
...
- Codex 后续所有修改必须同时遵守根目录 `CLAUDE.md` 和 `AGENTS.md`。
- 两份文件内容冲突时，以 `docs/source/requirements.md` 和更严格的规则为准，不允许降低、删除或绕过 `requirements.md` 的要求。
- `AGENTS.md` 必须持续完整保留 `CLAUDE.md` 的约束；更新 `CLAUDE.md` 时必须同步检查并更新 `AGENTS.md`。
