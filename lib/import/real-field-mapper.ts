...
// status 第一版不猜测业务含义，统一 raw_status_<value>
function mapTaskStatus(value: number | null): string {
  if (value === null || value === undefined) return 'raw_status_null'
...
export function mapRealTask(
  ...
  return {
    ...
    task_type: mapTaskType(source.task_type as number),
    status: mapTaskStatus(source.status as number),
    phase: null,
    priority: null,
