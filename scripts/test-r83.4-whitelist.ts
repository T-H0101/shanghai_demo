/**
 * R.83.4 Whitelist Self-Check
 *
 * 验证 lib/sync/package-schema.ts 的白名单结构：
 * - ALLOWED_PACKAGE_TABLES 共 73 张 (13 原始 + 15 R.83.1 + 15 R.83.2 + 15 R.83.3 + 15 R.83.4)
 ...
 * - 15 张 R.83.2 表全部仍存在
 * - 15 张 R.83.3 表全部仍存在
 * - 15 张 R.83.4 表全部存在 (存储卷 + 调度/接口 + 设备业务族)
 * - 无重复
 * - 末尾 15 张 (positions 58-72) 顺序与内容完全匹配 R.83.4 batch
 */
...
console.log('R.83.4 Whitelist Self-Check')
console.log('===========================\n')

// 1. 总长度 ≥ 73 (R.83.5 已扩到 88,本测试只保证 R.83.4 之后的最小集)
check(
  'ALLOWED_PACKAGE_TABLES.length >= 73 (R.83.4 baseline)',
  ALLOWED_PACKAGE_TABLES.length >= 73,
  `actual=${ALLOWED_PACKAGE_TABLES.length} (expected >=73; 后续 sprint 可能扩大)`
)
...
check(
  'all 15 R.83.4 tables present in ALLOWED_PACKAGE_TABLES',
  missing834.length === 0,
...
check(
  'all 73 entries match /^tbl_[a-z0-9_]+$/',
  invalid.length === 0,
...
// 10. positions 58-72 完全是 15 张 R.83.4 表 (按顺序)
const tail = ALLOWED_PACKAGE_TABLES.slice(58, 73)
const expectedTail = R834_TABLES
const tailMatch =
  tail.length === expectedTail.length &&
  tail.every((t, i) => t === expectedTail[i])
check(
  'positions 58-72 are exactly the 15 R.83.4 tables (in order)',
  tailMatch,
  `actual=[${tail.join(',')}] expected=[${expectedTail.join(',')}]`
)

// 11. 健全性: 13 + 15*4 = 当前基础集合,后续 sprint 可扩大但应 >= 73
check(
  'sanity: R.83.4 baseline (13+15*4) <= actual length',
