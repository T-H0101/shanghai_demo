 ...
 *
 * Sprint R.83.6 Task 3 — system config family CRUD over center DB
 * unified_sys_configs (primary table; lists 4 unified_* tables in sourceTables).
 *
 * Tables:
...
const TARGET_TABLE = "unified_sys_configs"
const SOURCE_TABLES = [
  "unified_sys_configs",
  "unified_sys_envs",
  "unified_meta_datas",
...
export async function GET(req: NextRequest) {
  const traceId = `r83-6-system-config-${Date.now()}`
  try {
