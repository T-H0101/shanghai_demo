/**
 * Sprint R.83.4 Task 5 — 2 CRUD endpoints self-check (volume + schedule)
 *
 * Exercises GET/POST/PUT/DELETE across 2 new resources:
 *   - storage  (unified_volume_groups)
 *   - ops      (unified_schedule_jobs)
 *
 * Total checks (≥12):
 *   - 2 endpoints × 5 happy-path = 10
 *   - 2 negative-path DELETE (missing params) = 2
 *
 * Usage:
 *   pnpm exec tsx app/api/volume/__tests__/self-check.ts
 */
...
const TEST_RECORD_ID = process.env.TEST_RECORD_ID ?? "test-r83-4-selfcheck-1"
...
async function main() {
  ...
  for (const resource of ["volume/storage", "schedule/ops"]) {
