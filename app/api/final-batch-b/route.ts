/**
 * GET/POST/PUT/DELETE /api/final-batch-b
 *
 * Sprint R.83.9 Task 3 — final batch B: 接收单明细 + 槽位分区 + 下载等待族 4 张
 * unified_receipt_file_details (primary table; lists 4 tables in sourceTables).
 *
...
const TARGET_TABLE = "unified_receipt_file_details"
const SOURCE_TABLES = [
...
export async function GET(req: NextRequest) {
  const traceId = `r839-final-batch-b-${Date.now()}`
  try {
